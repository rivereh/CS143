...
public class IntTree {
    ...
    // pre : max > 0
    // post: constructs a with the given number of nodes
    ...
    private TreeNode<Integer> buildTree(int n, int levels) {
        if (n > levels) {
            ...
            // generates a new node with a random value -50 to 50 and the right/left generated above
            return new TreeNode<Integer>(rand.nextInt(101) - 50, left, right);
        }
    }
    ...
    public int countOddValues() {
      return countOddValues(overallRoot);
    }
    
    private int countOddValues(TreeNode<Integer> root) {
      if (root == null)
         return 0;
      else if ((int) root.data % 2 != 0)
         return 1 + countOddValues(root.left) + countOddValues(root.right);
      else
         return 0 + countOddValues(root.left) + countOddValues(root.right);
    }
